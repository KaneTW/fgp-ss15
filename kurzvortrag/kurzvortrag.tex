    ...
    % then introduce frp
    \begin{itemize}
    \item Simple approach: call a function periodically that does everything
    \end{itemize}
    \inputminted{python}{imperative.py}
    \end{frame}
    \begin{frame}{A reactive approach}
    % hard to specify different timers
    % etc etc
    \begin{itemize}
    \item Moving away from single tick procedure
    \item Independent sampling rates and more modular code
    \end{itemize}
    ...
    \begin{frame}{Functional reactive programming}
        \begin{itemize}
            ...
            \item<+- | alert@+> \(\Rightarrow\) Functional Reactive Programming % one of the flavors of it, anyway
